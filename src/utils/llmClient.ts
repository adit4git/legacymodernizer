...
export function makeLlmClient(cfg: vscode.WorkspaceConfiguration, selection?: LlmSelection): LlmClient {
  const provider = selection?.provider || cfg.get<string>('modelProvider') || 'vscode-copilot';
  if (provider === 'vscode-copilot') return new VsCodeCopilotClient(cfg, selection?.model);
  if (provider === 'claude-sonnet')  return new ClaudeSonnetClient(cfg, selection?.model);
  return new OpenAiCodexClient(cfg, selection?.model);
}

// ---------- VS Code Language Model API (uses the user's Copilot subscription) ----------
class VsCodeCopilotClient implements LlmClient {
  private family: string;
  constructor(cfg: vscode.WorkspaceConfiguration, familyOverride?: string) {
    this.family = familyOverride || cfg.get<string>('copilotModelFamily') || 'gpt-4o';
  }
  ...
  async complete(args: CompleteArgs): Promise<CompleteResult> {
    ...
    if (models.length === 0) {
      throw new Error(
        'No Copilot language models available. Install GitHub Copilot + Copilot Chat, sign in, ' +
    ...
    for (const m of args.messages) {
      ...
      if (m.role === 'user') messages.push(vscode.LanguageModelChatMessage.User(content));
      else messages.push(vscode.LanguageModelChatMessage.Assistant(content));
    }

    // Translate tool schemas into VS Code's tool definitions.
    const tools: vscode.LanguageModelChatTool[] = (args.tools || []).map(t => ({
      name: t.name,
    ...
    try {
      response = await model.sendRequest(
        messages,
        { tools, justification: 'Legacy Modernizer is converting your code.' } as any,
        cancel.token
      );
    } catch (err: any) {
...
class ClaudeSonnetClient implements LlmClient {
  ...
  constructor(cfg: vscode.WorkspaceConfiguration, modelOverride?: string) {
    this.apiKey = cfg.get<string>('anthropicApiKey') || process.env.ANTHROPIC_API_KEY || '';
    this.model = modelOverride || cfg.get<string>('anthropicModel') || 'claude-sonnet-4-6';
    this.maxTokens = Math.max(256, cfg.get<number>('anthropicMaxTokens', 8192));
    this.retryAttempts = Math.max(1, cfg.get<number>('anthropicRetryAttempts', 4));
    this.retryBaseMs = Math.max(250, cfg.get<number>('anthropicRetryBaseMs', 1500));
    if (!this.apiKey) throw new Error('Anthropic API key not configured (modernizer.anthropicApiKey).');
  }
  ...
  async complete(args: CompleteArgs): Promise<CompleteResult> {
    ...
    if (!res || !res.ok) throw new Error(`Anthropic request failed after retries: ${lastErrText}`);
...
class OpenAiCodexClient implements LlmClient {
  ...
  private model: string;
  constructor(cfg: vscode.WorkspaceConfiguration, modelOverride?: string) {
    this.apiKey = cfg.get<string>('openaiApiKey') || process.env.OPENAI_API_KEY || '';
    this.model = modelOverride || cfg.get<string>('openaiModel') || 'gpt-4.1';
    if (!this.apiKey) throw new Error('OpenAI API key not configured (modernizer.openaiApiKey).');
  }
...
function stringifyMessageContent(content: string | any[]): string {
