...
import { LlmClient, LlmSelection, makeLlmClient } from '../utils/llmClient';
...
export type StepStatus = 'pending' | 'running' | 'done' | 'awaiting-review' | 'failed';
...
interface PlannedFileEntry {
  path: string;
  type?: string;
  name?: string;
}

interface ApiPlanSnapshot {
  generatedAt?: string;
  archHash?: string;
  files: PlannedFileEntry[];
}
...
export class Orchestrator {
  private steps: PipelineStep[] = JSON.parse(JSON.stringify(DEFAULT_STEPS));
  ...
  onStateChange(cb: () => void) { this.listeners.push(cb); }
  private notify() { this.listeners.forEach(l => l()); }
  private setStatus(id: string, status: StepStatus, detail?: string, artifactPath?: string) {
    const s = this.steps.find(x => x.id === id);
    if (s) { s.status = status; s.detail = detail; s.artifactPath = artifactPath; this.notify(); }
  }
  private getStep(id: string): PipelineStep | undefined {
    return this.steps.find(s => s.id === id);
  }
  private stepLabel(id: string): string {
    return this.getStep(id)?.label || id;
  }
  private ensureStepsDone(targetId: string, prerequisiteIds: string[], detail?: string): boolean {
    ...
    vscode.window.showWarningMessage(
      `${this.stepLabel(targetId)} is blocked. Complete ${missingLabels.join(' and ')} first.`
    );
    return false;
  }
  private ensureStepReadyForReview(reviewStepId: string, upstreamId: string): boolean {
    const upstreamStatus = this.getStep(upstreamId)?.status;
    if (upstreamStatus === 'awaiting-review' || upstreamStatus === 'done') return true;
    this.setStatus(reviewStepId, 'pending', `Blocked: ${this.stepLabel(upstreamId)} must run first`);
    vscode.window.showWarningMessage(
      `${this.stepLabel(reviewStepId)} is blocked. Run ${this.stepLabel(upstreamId)} first.`
    );
    return false;
  }
  ...
  legacyRoot() { return this.cfg().get<string>('legacyRoot') || ''; }
  targetRoot() { return this.cfg().get<string>('targetRoot') || ''; }
  archFile()   { return this.cfg().get<string>('targetArchitectureFile') || ''; }
  uiFw()       { return this.cfg().get<string>('targetUiFramework') || 'react'; }
  maxIter()    { return this.cfg().get<number>('maxIterations') || 40; }

  llm(): LlmClient {
    return makeLlmClient(this.cfg());
  }

  llmForStep(stepId: string): LlmClient {
    const modelFor = this.cfg().get<Record<string, string>>('modelFor') || {};
    const raw = modelFor[stepId];
    if (!raw) return this.llm();

    const selection = this.parseStepModelOverride(raw);
    if (!selection) {
      this.log(
        `[config] Invalid modernizer.modelFor.${stepId}="${raw}". ` +
    ...
    return makeLlmClient(this.cfg(), selection);
  }

  critiquePassEnabled(): boolean {
    return this.cfg().get<boolean>('enableCritiquePass', true);
  }

  private modernizerWorkDir(): string {
    return path.join(this.targetRoot(), '_modernizer');
  }

  private timestampToken(d = new Date()): string {
  ...
  private listArtifactsByPattern(root: string, pattern: RegExp): string[] {
  ...
  private latestInventoryFile(root: string): string | undefined {
    return this.listArtifactsByPattern(root, /^inventory(?:-\d{8}-\d{6})?\.json$/i)[0];
  }

  private latestDocsFile(root: string): string | undefined {
    return this.listArtifactsByPattern(root, /^LEGACY_DOCUMENTATION(?:-\d{8}-\d{6})?\.md$/i)[0];
  }

  private apiPlanPath(apiRoot: string): string {
    return path.join(apiRoot, '_modernizer', 'api-conversion-plan.json');
  }

  private loadApiPlan(planPath: string): ApiPlanSnapshot | undefined {
  ...
  private planCoverage(writeRoot: string, entries: PlannedFileEntry[]): { existingCount: number; missing: PlannedFileEntry[] } {
    const missing = entries.filter((entry) => {
      const resolved = resolvePlannedPath(entry.path, writeRoot);
  ...
  private computePlanContextHash(archMd: string, skillMd: string): string {
  ...
  private planAgeDays(generatedAt?: string): number | undefined {
  ...
  private cleanupPlannedArtifacts(writeRoot: string, planPath: string, entries: PlannedFileEntry[]): number {
    const writeRootAbs = path.resolve(writeRoot);
    let deleted = 0;
    for (const entry of entries) {
      const resolved = resolvePlannedPath(entry.path, writeRoot);
      if (!resolved) continue;
      if (!resolved.startsWith(`${writeRootAbs}${path.sep}`)) continue;
      try {
    ...
    if (fs.existsSync(resolvedPlanPath) && resolvedPlanPath.startsWith(`${writeRootAbs}${path.sep}`)) {
      try { fs.unlinkSync(resolvedPlanPath); } catch { /* ignore */ }
    }
    return deleted;
  }

  private parseStepModelOverride(raw: string): LlmSelection | undefined {
  ...
  async stepAnalyze(): Promise<void> {
    ...
    if (reusableFile) {
      const choice = await vscode.window.showInformationMessage(
        `${path.basename(reusableFile)} already exists. Reuse it for this run?`,
        'Reuse',
        'Regenerate'
      );
      if (choice === 'Reuse') {
    ...
    try {
      ...
      await runAgentLoop({
        ...
        userGoal:
          `Walk the legacy .NET codebase and write ${outName} to the writeRoot. ` +
  ...
  async stepGenerateDocs(): Promise<void> {
    ...
    if (!inventoryFile) {
      const choice = await vscode.window.showInformationMessage(
        'Generate Docs requires an inventory artifact from Analyze. Run Analyze now?',
        'Run Analyze',
        'Cancel'
      );
    ...
    if (reusableDoc) {
      const choice = await vscode.window.showInformationMessage(
        `${path.basename(reusableDoc)} already exists. Reuse it for this run?`,
        'Reuse',
        'Regenerate'
      );
      if (choice === 'Reuse') {
    ...
    try {
      ...
      await runAgentLoop({
        ...
        skillPath: path.join(this.context.extensionPath, 'skills', 'documentation-generator', 'SKILL.md'),
        userGoal:
          `Using ${inventoryName} plus source reading, write ${outName} to the writeRoot. ` +
      ...
      if (!fs.existsSync(outFile)) {
        throw new Error(
          `Documenter agent finished but did not write ${outName}. ` +
...
async stepReviewDocs(): Promise<void> {
  if (!this.ensureStepReadyForReview('reviewDocs', 'docs')) return;
  ...
  const choice = await vscode.window.showInformationMessage(
    `Human Gate #1: Have you reviewed ${docName}?`,
    { modal: true },
    'Approve & Continue', 'Re-generate', 'Cancel'
  );
  ...
  async stepConvertApi(): Promise<void> {
    if (!this.ensureStepsDone('convertApi', ['reviewDocs'], 'Blocked: approve Human Gate #1 (Docs) first')) return;
    if (!this.ensureSetup()) return;
    ...
    try {
      ...
      let conversionMode: 'full' | 'resume' = 'full';
      let resumeMissing: PlannedFileEntry[] = [];
      let resumeTotal = 0;
      ...
      if (existingPlan && existingPlan.files.length > 0) {
        ...
        if (hashMismatch) cautionParts.push('Architecture/skill hash changed');
        if (stalePlan) cautionParts.push(`Plan is ${Math.round(ageDays!)} days old`);
        const cautionSuffix = cautionParts.length > 0 ? ` (${cautionParts.join('; ')})` : '';

        this.log(
          `[apiConverter] found existing plan ${path.basename(planPath)} ` +
        ...
        if (coverage.missing.length === 0) {
          const choice = await vscode.window.showInformationMessage(
            `API conversion plan already appears complete.${cautionSuffix ? ` ${cautionParts.join('. ')}.` : ''} Reuse existing generated API?`,
            'Reuse',
            'Regenerate'
          );
          if (choice === 'Reuse') {
          ...
          const choice = await vscode.window.showInformationMessage(
            `Found existing API plan with ${coverage.missing.length} missing of ${existingPlan.files.length}.${extraWarning} Resume from missing files?`,
            'Resume',
            'Regenerate',
            'Cancel'
          );
      ...
      const fullGoal =
        `Generate a complete Spring Boot API in ${apiRoot} from the legacy .NET source.\n\n` +
      ...
      const resumeGoal =
        `Resume an interrupted Spring Boot API conversion in ${apiRoot}.\n\n` +
  ...
  async stepConvertUi(): Promise<void> {
    if (!this.ensureStepsDone('convertUi', ['reviewDocs'], 'Blocked: approve Human Gate #1 (Docs) first')) return;
    if (!this.ensureSetup()) return;
    ...
    try {
      ...
      const written = copyTemplates(tmpl, uiRoot, vars);
      ...
      await runAgentLoop({
        ...
        userGoal:
          `Project scaffolding already exists under ${uiRoot}: package.json, tsconfig.json, vite.config.ts (or angular.json), ` +
  ...
  async stepReviewCode(): Promise<void> {
    if (!this.ensureStepsDone('reviewCode', ['convertApi', 'convertUi'], 'Blocked: complete API + UI conversion first')) return;

    const choice = await vscode.window.showInformationMessage(
      'Human Gate #2: Review generated Spring Boot + SPA code in target folder.',
      { modal: true },
      'Approve & Continue', 'Re-generate API', 'Re-generate UI', 'Cancel'
    );
    if (choice === 'Approve & Continue') this.setStatus('reviewCode', 'done', 'Approved');
    else if (choice === 'Re-generate API') { this.setStatus('convertApi','pending'); await this.stepConvertApi(); }
    else if (choice === 'Re-generate UI')  { this.setStatus('convertUi','pending');  await this.stepConvertUi();  }
    else this.setStatus('reviewCode', 'pending', 'Cancelled');
  }

  async stepGenerateTests(): Promise<void> {
    if (!this.ensureStepsDone('tests', ['reviewCode'], 'Blocked: approve Human Gate #2 (Code) first')) return;
    if (!this.ensureSetup()) return;
    ...
    try {
      await runAgentLoop({
        ...
        userGoal:
          'Generate JUnit 5 + Mockito unit tests AND Spring Boot @SpringBootTest integration tests for the API ' +
  ...
  async stepGenerateCicd(): Promise<void> {
    if (!this.ensureStepsDone('cicd', ['tests'], 'Blocked: complete test generation first')) return;
    if (!this.ensureSetup()) return;
    ...
    try {
      ...
      const written = copyTemplates(tmpl, deployRoot, vars);
      ...
      await runAgentLoop({
        ...
        userGoal:
          `Most CI/CD manifests already exist as templates under ${deployRoot}. ` +
...
async stepReviewCicd(): Promise<void> {
  if (!this.ensureStepReadyForReview('reviewCicd', 'cicd')) return;

  const choice = await vscode.window.showInformationMessage(
    'Human Gate #3: Review OpenShift CI/CD manifests in deploy/.',
    { modal: true },
    'Approve & Continue', 'Re-generate', 'Cancel'
  );
  if (choice === 'Approve & Continue') {
  ...
  async runFullPipeline(): Promise<void> {
    ...
    if (this.steps.find(s => s.id === 'reviewDocs')!.status !== 'done') return;

    await this.stepConvertApi();
    await this.stepConvertUi();
    await this.stepReviewCode();
    if (this.steps.find(s => s.id === 'reviewCode')!.status !== 'done') return;
