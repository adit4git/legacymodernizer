...
const jira_1 = require("../mcp/jira");
const bitbucket_1 = require("../mcp/bitbucket");
const agentLoop_1 = require("../orchestrator/agentLoop");
function registerAllCommands(ctx, orch) {
    const reg = (id, fn) => ctx.subscriptions.push(vscode.commands.registerCommand(id, fn));
    reg('modernizer.openMenu', () => vscode.commands.executeCommand('workbench.view.extension.legacyModernizer'));
    ...
    reg('modernizer.defects.fetchJira', async () => {
        const jira = new jira_1.JiraMcp(orch);
        const issues = await jira.fetchOpenDefects();
        if (!issues.length) {
            vscode.window.showInformationMessage('No open defects found.');
            return;
        }
        const pick = await vscode.window.showQuickPick(issues.map(i => ({ label: `${i.key}  ${i.summary}`, description: i.priority, detail: i.description, issue: i })), { placeHolder: 'Select a Jira defect to resolve' });
        if (pick)
            ctx.workspaceState.update('modernizer.selectedDefect', pick.issue);
    });
    reg('modernizer.defects.resolve', async () => {
        const issue = ctx.workspaceState.get('modernizer.selectedDefect');
        if (!issue) {
        ...
        await (0, agentLoop_1.runAgentLoop)({
            ...
            userGoal: `Resolve Jira defect ${issue.key}: ${issue.summary}\n\nDESCRIPTION:\n${issue.description}\n\n` +
        ...
        const bb = new bitbucket_1.BitbucketMcp(orch);
    ...
    reg('modernizer.config.modelProvider', async () => {
        const pick = await vscode.window.showQuickPick(['vscode-copilot', 'claude-sonnet', 'openai-codex'], { placeHolder: 'Choose model provider' });
        if (pick)
            await vscode.workspace.getConfiguration('modernizer')
    ...
    reg('modernizer.listCopilotModels', async () => {
        ...
        if (models.length === 0) {
            vscode.window.showWarningMessage('No Copilot models. Install GitHub Copilot + Copilot Chat, sign in, send one Copilot Chat message to grant consent, then re-run.');
            return;
        }
        const lines = models.map(m => `• ${m.vendor}/${m.family}  id=${m.id}  maxInput=${m.maxInputTokens}`).join('\n');
