...
class ModernizerMenuProvider {
    ...
    html() {
        ...
        const legacy = cfg.get('legacyRoot') || '(not set)';
        const target = cfg.get('targetRoot') || '(not set)';
        const arch = cfg.get('targetArchitectureFile') || '(not set)';
        const ui = cfg.get('targetUiFramework') || 'react';
        const model = cfg.get('modelProvider') || 'vscode-copilot';
        return /* html */ `
<!DOCTYPE html>
<html>
<head>
...
exports.ModernizerMenuProvider = ModernizerMenuProvider;
