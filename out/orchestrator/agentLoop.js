...
async function runAgentLoop(opts) {
    ...
    const critiqueEnabled = critiquePass ?? orchestrator.cfg().get('enableCritiquePass', true);
    const historyTurns = Math.max(1, orchestrator.cfg().get('historyTurns', 12));
    const toolResultMaxChars = Math.max(2_000, orchestrator.cfg().get('toolResultMaxChars', 30_000));
    const skillPromptMaxChars = Math.max(4_000, orchestrator.cfg().get('skillPromptMaxChars', 45_000));
    const archPromptMaxChars = Math.max(4_000, orchestrator.cfg().get('archPromptMaxChars', 30_000));
    const interRequestDelayMs = Math.max(0, orchestrator.cfg().get('interRequestDelayMs', 1_000));
    const readFileDefaultMaxBytes = Math.max(10_000, orchestrator.cfg().get('readFileDefaultMaxBytes', 100_000));
    const readFileHardMaxBytes = Math.max(readFileDefaultMaxBytes, orchestrator.cfg().get('readFileHardMaxBytes', 250_000));
    ...
    for (let i = 0; i < maxIterations; i++) {
        ...
        if (interRequestDelayMs > 0 && i > 0)
            await sleep(interRequestDelayMs);
        const response = await llm.complete({ system, messages: promptMessages, tools: exports.TOOL_SCHEMAS });
        if (response.text)
            lastFreeText = response.text;
        ...
        for (const call of response.toolCalls) {
            try {
                ...
                if (call.name === 'finish') {
                    lastFreeText = call.input?.summary || lastFreeText;
                    finished = true;
                }
            }
            catch (e) {
        ...
        if (critique.toolCalls) {
            for (const call of critique.toolCalls) {
                try {
                    ...
                    orchestrator.log(`[${agent}] critique tool error: ${e.message}`);
                }
            }
        }
    }
...
function limitText(text, maxChars, marker) {
    if (text.length <= maxChars)
        return text;
    return `${text.slice(0, maxChars)}\n\n${marker} ${text.length - maxChars} chars omitted.`;
}
function windowMessages(messages, historyTurns) {
...
async function runPlanVerificationPasses(opts) {
    ...
    for (let pass = 1; pass <= 3; pass++) {
        ...
        const getMissing = () => planFiles.filter((entry) => {
            const resolved = (0, plannedPaths_1.resolvePlannedPath)(entry.path, opts.writeRoot);
        ...
        if (missing.length > 10)
            opts.orchestrator.log(`  ... and ${missing.length - 10} more`);
        const formatMissing = (rows, maxRows = rows.length) => rows
            .slice(0, maxRows)
            .map((entry) => `- ${entry.path}${entry.type ? ` (${entry.type})` : ''}`)
            .join('\n');
        const fixupGoal = `Your plan at ${planPath} lists ${planFiles.length} files but only ${planFiles.length - missing.length} ` +
        ...
        const fixupMessages = seedGoal ? [seedGoal] : [];
        ...
        for (let turn = 0; turn < fixupTurns && !fixupFinished; turn++) {
            if (opts.interRequestDelayMs > 0)
                await sleep(opts.interRequestDelayMs);
            ...
            if (!fixup.toolCalls || fixup.toolCalls.length === 0) {
                ...
                opts.orchestrator.log(`[${opts.agent}] fixup turn ${turn + 1}: no tool calls (streak ${noToolStreak}); ` +
                ...
                fixupMessages.push({
                    role: 'user',
                    content: 'You replied with text instead of tool calls. There are still missing files. ' +
...
function isAnthropicClient(llm) {
    return llm.name().startsWith('anthropic:');
}
function assistantContentForNoToolTurn(response, structuredToolMessages) {
    if (!structuredToolMessages)
        return response.text || '';
    return response.rawContent ?? buildAssistantContent(response);
}
function appendToolTurnMessages(messages, response, toolResults, structuredToolMessages) {
    if (structuredToolMessages) {
        messages.push({
            role: 'assistant',
...
function findPlanPath(agent, writeRoot) {
...
function planFileNamesForAgent(agent) {
    if (agent === 'apiConverter')
        return ['api-conversion-plan.json', 'conversion-plan.json'];
    if (agent === 'uiConverter')
        return ['ui-conversion-plan.json', 'conversion-plan.json'];
    return ['conversion-plan.json'];
}
function loadPlanFiles(planPath) {
    try {
...
async function dispatchTool(call, ctx) {
    ...
    switch (call.name) {
        case 'list_dir': {
            ...
            const files = await (0, glob_1.glob)(pattern, { cwd: base, nodir: true, dot: false });
        ...
        case 'search_text': {
            ...
            const files = await (0, glob_1.glob)(gl, { cwd: ctx.legacyRoot, nodir: true });
            ...
            for (const f of files) {
                try {
                    ...
                    text.split('\n').forEach((line, i) => {
                        if (re.test(line))
                            hits.push({ file: f, line: i + 1, text: line.trim().slice(0, 200) });
                    });
                }
                catch { /* skip */ }
                if (hits.length > 200)
        ...
        case 'write_file': {
            if (!ctx.writeFiles || !ctx.writeRoot)
                throw new Error('write_file disabled for this step');
